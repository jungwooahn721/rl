...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        pt_observation = ptu.from_numpy(observation)
        
        action_dist = self.forward(pt_observation)
                
        sampled_action = action_dist.sample()
        
        np_action = ptu.to_numpy(sampled_action)
    ...
    def update(self, observations, actions):
        ...
        sampled_action = actions_pred.rsample()
        
        loss = self.loss(sampled_action, actions)
        loss.backward()
        self.optimizer.step()
        #print(f'Training Loss: {ptu.to_numpy(loss)}')
        return {
