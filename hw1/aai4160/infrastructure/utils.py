...
def sample_trajectory(env, policy, max_path_length, render=False):
    ...
    ob =  env.reset()      #a # HINT: should be the output of resetting the env
    ...
    while True:
        ...
        obs.append(ob)
        ac = policy.get_action(ob) # HINT: query the policy's get_action function #a (obs) -> (ob)
        ac = ac[0]
        acs.append(ac)
...
def sample_trajectories(env, policy, min_timesteps_per_batch, max_path_length, render=False):
    ...
    while timesteps_this_batch < min_timesteps_per_batch:
        sampled_path = sample_trajectory(env, policy, max_path_length, render)
