...
class BCTrainer:
    ...
    def collect_training_trajectories(
        ...
        if itr == 0:                                                               
            if load_initial_expertdata is not None:     
                print(f"\nCollecting expert({load_initial_expertdata}) data to be used for training...")                           
            ...
            # (2)                                                                                                                        
                ...
                paths = utils.sample_n_trajectories(
                    self.env, 
                    collect_policy, 
                    self.params['batch_size_initial'], 
                    self.params['ep_len']
                    )
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, training_logs):
        ...
        if self.log_metrics:
            ...
            #logs[f"Eval_AverageReturn_bs{self.params['batch_size']}_nl{self.params['n_layers']}_width{self.params['size']}"] \
            #    = np.mean(eval_returns)    #a
            #logs[f"Eval_AverageReturn_nl{self.params['n_layers']}_bs{self.params['batch_size']}"] = np.mean(eval_returns)
            ...
            last_log = training_logs[-1]  # Only use the last log for now #a
